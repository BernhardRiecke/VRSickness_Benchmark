...
namespace HP.Omnicept.Unity
{
    ...
    public class GliaBehaviour : MonoBehaviour
    {
        ...
        {
            Debug.Log("here is the new one:" + heartRate);
        }
        */
        ...
        private bool BindToForegroundOmniceptRuntime()
        {
            if (!m_isBound)
            {
                using (var javaUnityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
                using (var currentActivity = javaUnityPlayer.GetStatic<AndroidJavaObject>("currentActivity"))
                using (var androidPlugin = new AndroidJavaObject("com.hp.omnicept.client.Setup", currentActivity))
                {
                    m_isBound =  androidPlugin.Call<bool>("bindToService");
                }
            }
            return m_isBound;
        }

        private void UnbindToForegroundOmniceptRuntime()
        {
            if (m_isBound)
            {
                using (var javaUnityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
                ...
                using (var androidPlugin = new AndroidJavaObject("com.hp.omnicept.client.Setup", currentActivity))
                {
                    androidPlugin.Call("unbindToService");
                    m_isBound = false;
                }
            }
        }
        ...
        private void Start()
        {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                m_connectTask = Task.Run(() =>
                {
                    ...
                    bool bound = BindToForegroundOmniceptRuntime();
                    if (bound)
                    {
                        Debug.Log("Bound To Omnicept Runtime");
                    }
                    else
                    {
                        Debug.Log("Failed to bind To Omnicept Runtime");
                    }
                    return bound;
                }).ContinueWith((bound) =>
                {
                    if (bound.Result)
                    {
                        ConnectToOmniceptRuntime(settings, sessionLicense);
                    }
                });
            }
            else
            ...
            m_subList = settings.GetSubscriptionList();
        }
        ...
        bool WantsToQuit()
        {
            if(Application.platform == RuntimePlatform.Android)
            {
                UnbindToForegroundOmniceptRuntime();
            }
            OnDestroy();
            return true;
        }
